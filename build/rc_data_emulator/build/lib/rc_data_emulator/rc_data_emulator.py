...
class DPRclient:
    ...
    def __init__(self, ip_address, ushmfile='', port=DPR_PORT, endianness='auto', timeout=None):
        ...
        # If auto endinness is provided, try to guess by
    ...
    def determine_endianness(self):
        ...
        send_val = 12345678
        self.set_raw(8, struct.pack('<I', send_val))
        return_data = self.get_raw(8, 4)
        return_val = struct.unpack('<I', return_data)[0]

        if send_val == return_val:
            endianness = 'little'
        else:
            endianness = 'big'
        return endianness
...
def test_dpr():
    ...
    print(ip_address)
    # ushmfile =  r'C:\Dev\Robots\Robots\PMAC Script Language\Global Includes\ushm definitions.pmh'
